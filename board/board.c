...
void SysTick_Configuration(void)
{
    ...
    nrfx_clock_enable();/* 配置 LFCLK 的时钟源为外部低速晶振 */
    nrfx_clock_lfclk_start();
    /* Set interrupt priority */
    NVIC_SetPriority(SysTick_IRQn, 0xf);

    /* Configure SysTick to interrupt at the requested rate. */
    nrf_systick_load_set(SystemCoreClock / RT_TICK_PER_SECOND);/* 设置SysTick->LOAD = 64M/RT_TICK_PER_SECOND */
    nrf_systick_val_clear();/* SysTick->VAL = 0 */
