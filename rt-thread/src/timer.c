...
void rt_timer_check(void)
{
    ...
    current_tick = rt_tick_get();/* 获取当前 tick 计数 */
    ...
    while (!rt_list_isempty(&_timer_list[RT_TIMER_SKIP_LIST_LEVEL - 1]))
    {
         ...
         * tick max.
         */
        if ((current_tick - t->timeout_tick) < RT_TICK_MAX / 2)/* 如果超时了，current_tick - t->timeout_tick< 0，D31是1，>7FFFFFFF */
        {
            RT_OBJECT_HOOK_CALL(rt_timer_enter_hook, (t));/* 钩子函数 */
